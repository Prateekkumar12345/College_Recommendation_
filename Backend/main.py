...
# Add this new class for generating visualizations
class VisualizationGenerator:
    def __init__(self, student_model, college_model):
        self.student_model = student_model
        self.college_model = college_model
    
    def generate_student_cluster_viz(self):
        """Generate student cluster visualization"""
        if not self.student_model.fitted:
            return None
            
        # Use first two PCA components for visualization
        pca_2d = PCA(n_components=2)
        pca_result_2d = pca_2d.fit_transform(self.student_model.pca_features)
        
        plt.figure(figsize=(10, 8))
        sns.scatterplot(
            x=pca_result_2d[:, 0], 
            y=pca_result_2d[:, 1], 
            hue=self.student_model.cluster_labels, 
            palette='viridis',
            s=100,
            alpha=0.7
        )
        ...
        return {
            ...
            'description': f'Students grouped into {self.student_model.num_clusters} clusters based on academic performance, preferences, and goals'
        }
    
    def generate_college_cluster_viz(self):
        """Generate college cluster visualization"""
        if not self.college_model.fitted:
            return None
            
        plt.figure(figsize=(12, 8))
        sns.scatterplot(
            x=self.college_model.pca_features[:, 0],
            y=self.college_model.pca_features[:, 1],
            hue=self.college_model.cluster_labels,
            palette='Set1',
            s=120,
            alpha=0.8
        )
        ...
        plt.xlabel('First Principal Component')
        plt.ylabel('Second Principal Component')
        plt.legend(title='College Cluster')
        plt.grid(True, alpha=0.3)
        ...
        return {
            'chart_type': 'scatter',
            'data': {'image': graphic},
            'title': 'College Clustering Analysis',
            'description': f'Colleges grouped into {self.college_model.num_clusters} clusters based on type, location, fees, and offerings'
        }
    
    def generate_recommendation_similarity_viz(self, recommendations_df, student_dict):
        """Generate similarity scores visualization"""
        ...
        college_names = [name[:30] + '...' if len(name) > 30 else name 
                        for name in recommendations_df['College Name']]
        similarities = recommendations_df['Similarity'].values
        
        bars = plt.bar(range(len(college_names)), similarities, 
                      color=['#3498db', '#e74c3c', '#2ecc71', '#f39c12', '#9b59b6'][:len(college_names)])
        
        plt.title('College Recommendation Similarity Scores')
        plt.xlabel('Recommended Colleges')
        plt.ylabel('Similarity Score')
        plt.xticks(range(len(college_names)), college_names, rotation=45, ha='right')
        plt.ylim(0, 1)
        ...
        return {
            ...
            'description': 'How well each recommended college matches your profile (higher is better)'
        }
    
    def generate_student_profile_radar(self, student_dict):
        """Generate radar chart for student profile"""
        # Normalize student data for radar chart
        categories = ['Academic Performance', 'Budget Flexibility', 'Location Preference', 
                     'Technical Skills', 'Communication']
        
        # Calculate scores (0-1 scale)
        academic_score = (student_dict['Marks_10th'] + student_dict['Marks_12th']) / 200
        budget_flexibility = min(student_dict['Budget'] / 1000000, 1.0)  # Normalize to max 10L
        
        # Location preference (higher if flexible)
        location_score = 0.8 if student_dict['Preferred Location'].lower() in ['any', 'flexible'] else 0.5
        
        # Technical skills based on certifications
        tech_keywords = ['python', 'programming', 'ai', 'web', 'data', 'coding']
        cert_text = student_dict['Certifications'].lower()
        tech_score = min(sum(1 for keyword in tech_keywords if keyword in cert_text) / len(tech_keywords), 1.0)
        
        # Communication score based on English proficiency
        comm_mapping = {'Poor': 0.25, 'Average': 0.5, 'Good': 0.75, 'Excellent': 1.0}
        comm_score = comm_mapping.get(student_dict['English Proficiency'], 0.5)
        
        values = [academic_score, budget_flexibility, location_score, tech_score, comm_score]
        ...
        ax.plot(angles, values, 'o-', linewidth=2, label='Your Profile', color='#3498db')
        ax.fill(angles, values, alpha=0.25, color='#3498db')
        ax.set_xticks(angles[:-1])
        ax.set_xticklabels(categories)
        ax.set_ylim(0, 1)
        ax.set_title('Your Student Profile Analysis', size=16, pad=20)
        ax.grid(True)
        ...
        return {
            ...
            'description': 'Your profile strengths across different dimensions'
        }
...
# Update the startup event to initialize the visualization generator
...
async def startup_event():
    ...
    # Initialize visualization generator if models are loaded
    global viz_generator
    if student_model is not None and college_model is not None:
        viz_generator = VisualizationGenerator(student_model, college_model)
        print("✅ Visualization generator initialized")
    else:
        print("⚠️ Visualization generator not initialized - models not available")
...
async def get_visualizations(student_profile: StudentProfile):
    """Generate visualizations for the student profile and recommendations"""
    
    if not viz_generator:
        raise HTTPException(
            status_code=503,
            detail="Visualization service not available. Models not loaded."
        )
    ...
    try:
        ...
        # Generate student profile radar chart
        profile_viz = viz_generator.generate_student_profile_radar(student_dict)
        if profile_viz:
            visualizations.append(profile_viz)
        
        # Generate student clustering visualization
        student_cluster_viz = viz_generator.generate_student_cluster_viz()
        if student_cluster_viz:
            visualizations.append(student_cluster_viz)
        
        # Generate college clustering visualization
        college_cluster_viz = viz_generator.generate_college_cluster_viz()
        if college_cluster_viz:
            visualizations.append(college_cluster_viz)
        
        # Generate recommendation similarity if we have recommendations
        if recommender:
            try:
                recommendations_df = recommender.recommend(student_dict, top_n=5)
                if len(recommendations_df) > 0:
                    similarity_viz = viz_generator.generate_recommendation_similarity_viz(
                        recommendations_df, student_dict
                    )
                    if similarity_viz:
                        visualizations.append(similarity_viz)
            except Exception as e:
                print(f"Warning: Could not generate similarity visualization: {e}")
...
# Add endpoint for individual chart types
@app.get("/visualize/clusters")
async def get_cluster_visualizations():
    """Get only the clustering visualizations"""
    if not viz_generator:
    ...
    try:
        ...
        return {
            "success": True,
            "visualizations": visualizations,
            "message": f"Generated {len(visualizations)} cluster visualizations"
        }
    
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Error generating cluster visualizations: {str(e)}"
        )
